...
解决: Leader Epoch(0.11引入, KIP-101)
  每次选举递增epoch
  副本记录epoch→LEO映射(存于leader-epoch-checkpoint文件)
  新Leader上任先发LeaderEpochRequest查Follower日志
  Follower返回该epoch的LEO → Leader按LEO截断
  → 防止基于错乱的HW截断(解决HW截断丢数据)
...
│ LSO      │ Last Stable Offset, 事务模式下最早的未完成事务位移 │
│          │ LSO ≤ HW, read_committed消费者只能读LSO以下 │
└──────────┴──────────────────────────────────────┘
