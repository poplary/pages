...
```java
// RecordAccumulator 核心结构
public final class RecordAccumulator {
    // 每个分区对应一个批次队列
    private final ConcurrentMap<TopicPartition, Deque<ProducerBatch>> batches;
    
    // 缓冲池(复用 ByteBuffer, 减少 GC)
...
public final class ProducerBatch {
    ...
    private int batchBytes;        // 批次大小
    private Records records;        // 压缩后的消息集合
    private final ProduceRequestResult produceRequestResult;
}
```
...
```java
// 事务生产者配置
props.put("transactional.id", "my-tx-id");  // 必须全局唯一
props.put("enable.idempotence", true);        // 事务隐含幂等
