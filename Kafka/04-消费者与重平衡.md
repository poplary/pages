...
#### JoinGroup/SyncGroup 流程（经典协议）

```text
1. Consumer发JoinGroup → Coordinator
   (携带: group.id, subscription, assignor)
2. Coordinator等待所有成员Join, 选Leader(第一个Join的Consumer)
   ⚠️ Leader是Consumer不是Coordinator, 负责计算分配
3. Leader用Assignor计算全量分区分配方案
4. Leader通过SyncGroup把方案发给Coordinator
5. Coordinator校验一致性, 通过SyncGroup响应
   把各自分配发给所有成员
6. Consumer从SyncGroup响应获自己的分区分配, 开始消费

⚠️ 经典协议: 步骤1-5期间所有Consumer暂停消费(STW)
   Leader单点计算 → 大组时Join等待可达数分钟

4.0 KIP-848: 服务端(Coordinator)直接计算分配
  → 无需客户端选Leader, 无需两轮SyncGroup
  → 增量协同, 仅变化分区暂停, 消除STW
```
